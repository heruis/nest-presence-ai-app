...
import { allStates, type HomeState } from "@/lib/data";
import { cn } from "@/lib/cn";

const accentMap: Record<string, { ring: string; bg: string; text: string; bar: string }> = {
  amber: { ring: "ring-amber-400/30", bg: "bg-amber-400/10", text: "text-amber-300", bar: "bg-amber-400" },
  green: { ring: "ring-emerald-400/30", bg: "bg-emerald-400/10", text: "text-emerald-300", bar: "bg-emerald-400" },
  blue: { ring: "ring-sky-400/30", bg: "bg-sky-400/10", text: "text-sky-300", bar: "bg-sky-400" },
  cyan: { ring: "ring-cyan-400/30", bg: "bg-cyan-400/10", text: "text-cyan-300", bar: "bg-cyan-400" },
  violet: { ring: "ring-violet-400/30", bg: "bg-violet-400/10", text: "text-violet-300", bar: "bg-violet-400" },
  indigo: { ring: "ring-indigo-400/30", bg: "bg-indigo-400/10", text: "text-indigo-300", bar: "bg-indigo-400" },
};
    ...
    >
      <div className="px-5 pt-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-white/55">Currently inferred</p>
              <p className="text-[18px] font-semibold tracking-tight text-white">{state.name}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="tabular-nums text-[22px] font-semibold text-white">{state.confidence}%</p>
      ...
      <div className="mt-4 border-t border-white/6 px-3 py-3">
        <p className="px-2 pb-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-white/45">
          Signal fusion · 4 of 4 sources agree
        </p>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between">
          ...
          <p className="tabular-nums text-[12px] font-semibold text-white/70">{state.confidence}%</p>
        </div>
        <p className="text-[11px] text-white/50">Last fired · {state.inferredAt}</p>
        <div className="mt-1.5 h-[3px] w-full overflow-hidden rounded-full bg-white/6">
          <div className={cn("h-full rounded-full", a.bar)} style={{ width: `${state.confidence}%` }} />
        </div>
      </div>
    ...
    <div className="aurora h-full w-full overflow-y-auto no-scrollbar pb-32">
      ...
      <div className="px-6 pt-12">
        <div className="flex items-center gap-2 text-[#8ab4f8]">
          ...
          <span className="text-[11px] font-semibold uppercase tracking-[0.16em]">Nest Presence AI</span>
        </div>
        <h1 className="mt-1 text-[28px] font-semibold leading-tight tracking-tight text-white">
          Six states, four signals,
          <br />
          one home that knows you.
        </h1>
        <p className="mt-2 text-[13px] leading-snug text-white/60">
          Presence AI fuses geofencing, on-device camera ML, your Calendar, and learned time-of-day patterns to
          decide what your home should do next.
        </p>
      </div>
      ...
      <div className="mx-4 mt-4 rounded-2xl bg-gradient-to-br from-[#1a4a8a]/30 to-[#16191c] p-4 ring-1 ring-white/5">
        <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#8ab4f8]">Trust thresholds</p>
        <ul className="mt-2 space-y-1.5 text-[12px] text-white/75">
          <li>· Security actions (lock, arm) require ≥ 90% confidence</li>
          <li>· Comfort actions (lights, climate) require ≥ 75%</li>
          <li>· Below threshold → confirmation card, never auto-execute</li>
        </ul>
      </div>
    </div>
  );
